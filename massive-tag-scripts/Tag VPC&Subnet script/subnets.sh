...
# Script Function
# execute:
#             bash subnetstagging.sh [none | archivo.txt] [FILTER] [listtag.txt] [PROFILE]
...
if [ $TAG == "none" ]; then FILES=$(cat $1 | grep "^i")
  ...
  if [ $TAG == "all" ]; then
    aws ec2 describe-subnets --profile $PROFILE | grep "SubnetId" | sed 's/"SubnetId": "//g' | sed 's/",//g' | sed 's/ //g' > id_ec2_tag.txt
    FILES=$(cat id_ec2_tag.txt | grep "^s")
  else
    aws ec2 describe-subnets --filters "Name=tag:$TAG,Values=$VALUE" --profile $PROFILE | grep "SubnetId" | sed 's/"SubnetId": "//g' | sed 's/",//g' | sed 's/ //g' > id_ec2_tag.txt
    FILES=$(cat id_ec2_tag.txt | grep "^s")
  fi
fi
...
for f in $FILES
  do
    ...
    aws ec2 describe-subnets --subnet-ids $f --profile $PROFILE --output text | grep TAGS | cut -f 2 | sed -e 's/"//g' | sed -e 's/$//g' > tags_id.txt
    declare -a instancetaglist
    mapfile -t instancetaglist < tags_id.txt
    aws ec2 describe-subnets --subnet-ids $f --profile $PROFILE --output text | grep TAGS | cut -f 3 | sed -e 's/"//g' | sed -e 's/$//g' > values_id.txt
    declare -a instancevaluelist
    mapfile -t instancevaluelist < values_id.txt
    echo "TAG LIST de la subnet:${instancetaglist[@]} VALORES: ${instancevaluelist[@]}"
    ...
    aws ec2 describe-subnets --subnet-ids $f --profile $PROFILE --output text | grep TAGS
 done
