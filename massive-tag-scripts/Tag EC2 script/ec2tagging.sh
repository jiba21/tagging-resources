...
# Script Function
# execute:
#             bash ec2tagging.sh [none | archivo.txt] [FILTER] [listtag.txt] [PROFILE]
...
for f in $FILES
  do
    ...
    aws ec2 describe-instances --instance-ids $f --profile $PROFILE --output text | grep TAGS | cut -f 2 | sed -e 's/^/"/g' | sed -e 's/$/"/g' > tags_id.txt
    declare -a instancetaglist
    mapfile -t instancetaglist < tags_id.txt
    aws ec2 describe-instances --instance-ids $f --profile $PROFILE --output text | grep TAGS | cut -f 3 | sed -e 's/^/"/g' | sed -e 's/$/"/g' > values_id.txt
    ...
    aws ec2 describe-instances --instance-ids $f --profile $PROFILE --output text | grep TAGS
 done
